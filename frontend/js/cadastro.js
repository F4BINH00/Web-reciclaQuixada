...
botoesTipoUsuario.forEach(function (botao) {
  botao.addEventListener("click", function () {
    ...
    const textoTipo = nomeTipo
      ? nomeTipo.textContent.trim()
...
formularioCadastro.addEventListener("submit", async function (event) {
  ...
  try {
    ...
    const respostaAtualizacao = await fetch(
      `http://localhost:1337/api/users/${usuario.id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!respostaAtualizacao.ok) {
      ...
      alert(
        "Usuário criado, mas não foi possível salvar os dados complementares.",
      );
      return;
    }
